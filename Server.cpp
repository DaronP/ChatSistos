...
void * newConnection(const void * con)
{
	Server * serv = ((Server *)con);
	
	struct clientInfo requestD = serv->requestDel();
	char req[1500];
	serv->readReq(requestD.requestFD, req);
	...
	if(optn == 1)
	{
		usr = serv->registerUsr(cliMsg.synchronize(), requestD);
	}
	else
	{
		usr = "";
		...
		serv->sendRes(requestD.requestFD, &requestD.socketCli, clireq);
	...
	if(usr != "")
	{
		...
		clientInfo userInfo = serv->getUser(usr);
		...
		while(1)
		{
			int rdSize = serv->readReq(requestD.requestFD, wasd);
			...
			if(serv->sendRes(requestD.requestFD, &requestD.socketCli, nose) < -1)
			{
				cout << "Error sending response " << endl;
			}
			else
...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
    	...
    	pthread_create(&serverThread, NULL, &newConnection, this);
    }
